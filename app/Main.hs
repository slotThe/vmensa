...
import Network.HTTP.Conduit
    ( Manager, httpLbs, newManager, parseUrlThrow, responseBody, tlsManagerSettings
    )
...
main = do
    ...
    -- Get the specified date in ISO, as well as a printable format and create a
    -- new manager for handling network connections.
    ...
    -- Print out the results synchronously, so as to respect the desired order.
    traverse_ T.putStr mensen

{- Note [Async]
   ~~~~~~~~~~~~~~~~~~~~~~
   Here, we concurrently connect to the API, parse the necessary JSON and then
   create some pretty-printed text for each canteen.

   The function @forConcurrently@ creates a thread for every canteen, which
   would usually be a fire hazard and may cause your laptop to melt through your
   desk.  However, since the list of all canteens is rather small (even trying
   to show __everything__ there is would only be around 20 network connections),
   it was deemed "worth it" in this case.
-}
...
getMensa manager opts mensa@Mensa{ url } = catch
    do req <- parseUrlThrow (unpack url)
       maybe mensa
